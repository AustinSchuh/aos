...
TXUdpSocket::TXUdpSocket(const std::string &ip_addr, int port)
  ...
  ABSL_CHECK(inet_aton(ip_addr.c_str(), &destination_in.sin_addr) != 0)
      << ": invalid IP address " << ip_addr;

  ABSL_PCHECK(connect(fd_.get(), reinterpret_cast<sockaddr *>(&destination_in),
...
int RXUdpSocket::SocketBindListenOnPort(int port) {
  int fd;
  ABSL_PCHECK((fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) != -1);
  ...
  ABSL_PCHECK(bind(fd, reinterpret_cast<sockaddr *>(&bind_address),
...
int RXUdpSocket::Recv(void *data, int size) {
  int result;
  ABSL_PCHECK((result = recv(fd_.get(), static_cast<char *>(data), size, 0)) !=
              -1);
  return result;
}
