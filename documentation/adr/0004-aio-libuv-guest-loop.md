...
Accepted. Implemented in `aos/events/aio_uv.h` and `aos/events/aio_uv.cc`, with the platform half in `aos/events/aio_uv_posix.cc` and `aos/events/aio_uv_windows.cc` behind the seam in `aos/events/aio_uv_internal.h`; held to the `Aio` contract by `//aos/events:aio_test_uv` and covered past what that suite can reach by `aos/events/aio_uv_backend_test.cc`. Runs on Linux, macOS and Windows.
...
- **The only way to wait on a kernel object through libuv is `uv_poll` on a descriptor**, so everything AOS needs waking for has to be an fd. On Windows it is narrower still: `uv_poll` takes a socket, and nothing else.
...
5. **Timers are a descriptor carrying the deadline, watched with `uv_poll`, not `uv_timer_t`.** A `timerfd` on Linux; the descriptor keeps the nanosecond deadline AOS schedules with and is just another thing to poll — the same trade `EpollImpl` makes. On Windows, which has no descriptor to give libuv, they are a `TimerQueue` and one kernel timer that wakes the loop through its port; see below.
6. **The wakeup is `uv_poll` on a descriptor the `ThreadSignalReceiver` can be observed through** — its `signalfd` on Linux, which is what `EpollImpl` does with it too. On Windows it is the receiver's `Event`, watched through the loop's port the same way the timer is.
...
### Windows: the loop is a port, so it is waited on like one

libuv on Windows can poll a socket and nothing else, so neither a deadline nor a wakeup can be handed to it as a descriptor, and the substitution macOS makes has nothing to substitute with. What the loop _is_ there, though, is an I/O completion port — `uv_loop_t::iocp` — and ADR 0003's wait completion packets attach any waitable object's signal to any port, this borrowed one included. libuv dequeues a packet that carries no `OVERLAPPED` as a bare wake: the wait ends and nothing else happens. It uses such packets itself, in `uv__wake_all_loops()`, so the path is one libuv maintains rather than one this backend found.

So the Windows half keeps a `TimerQueue` — the container the IOCP and kqueue backends use, with the same `(deadline, sequence)` order — and one high-resolution kernel timer armed for its head, delivered as a bare wake. A legacy registration on a waitable `HANDLE` rather than a socket (a glib `GPollFD`, and the `ThreadSignalReceiver`'s `Event`, registered as an `OnReadable()` on that handle exactly as the IOCP backend registers it) is a packet on the handle, delivered the same way. A `uv_check_t`, which libuv runs after every wait, then asks the objects themselves: the timer is manual-reset, so it stays signalled until it is set again and a fire can be told from a wake that was somebody else's; a handle is asked with a zero-timeout `WaitForSingleObject()`. Every due timer is dispatched in order and the kernel timer re-pointed at what is left; a signalled handle is dispatched with its subscribed mask and re-armed only after the callback has had its chance to consume it — the IOCP backend's `HandleWatch` rules. The check handle is referenced only while AOS has a timer or a handle to wait for, so AOS with nothing scheduled keeps the owner's loop alive no more here than elsewhere.

Two things about libuv's own socket polling have to be read before they are believed. Its report is a snapshot: libuv re-submits its poll the moment it reports, a still-readable socket completes the new one at once, and if the caller consumes the data before the loop turns again, the next turn delivers readiness that is no longer there. And it only carries a peer's abrupt departure alongside readability: libuv asks AFD for exactly the events it was given, a subscription to disconnect alone is the graceful close alone, and the reset (`AFD_POLL_ABORT`) is requested only with readability and reported as it. Both have the answer the IOCP backend already uses: a registration that wants errors is watched for readability too, and every report is checked at dispatch, with a `MSG_PEEK` and a `select()` — data, EOF or error, and whether writability still holds. A hangup is reported as kqueue and IOCP report it: `kIn` to a registration that reads, so it can observe the zero-length read and unregister, and `kErr` to one that does not. Each of the three is pinned by a test that fails without it: `LegacyFdTest` and `EPollLikeBasicError` for the peek, `AbortiveResetReachesAnErrorOnlyRegistration` for the readability watch.

One thing that turned out not to be in play: libuv's slow poll, `select()` on a worker thread, is what it uses for sockets whose provider is not MSAFD. The `AF_UNIX` pair `Pipe` builds on this platform reports an MSAFD provider id, so it gets fast poll like TCP/IP does, and nothing AOS registers reaches the worker thread.

The seam between the halves is `UvPlatform` in `aio_uv_internal.h`: what to ask libuv to watch, how to read what it reports, which registrations the platform waits on itself, the timer, and the wakeup. `aio_uv.cc` carries no platform conditionals; the POSIX half is the timerfd and kqueue code that used to be inline there.
...
- **The backend runs on Linux, macOS and Windows, and the last is the most expensive.** On Linux the two things AOS needs waking for already are descriptors, a `timerfd` per timer and the receiver's own `signalfd`. macOS has neither, and a kqueue is itself a descriptor: one holding a single `EVFILT_TIMER` stands in for the timerfd and one holding a single `EVFILT_SIGNAL` for the signalfd, so below six helpers there is only ever an fd. ADR 0002 covers that substitution in full. Windows has no descriptor to offer at all and rides the loop's port instead — above — which costs a `uv_check_t` callback per turn, a `MSG_PEEK` or `select()` per readiness report, and a `WaitForSingleObject()` plus a packet re-arm per watched handle per turn, and it reaches into `uv_loop_t::iocp`, a field libuv's public header declares but does not document.

- **Callbacks still only ever run on the loop's thread**, which is what AOS assumes an event loop means. That is worth stating because Windows is the one platform here with threads in the picture at all. libuv's slow poll runs `select()` on a worker thread, but that thread only posts a completion back to the loop (`POST_COMPLETION_FOR_REQ` in `src/win/poll.c`); `poll_cb` is called from `uv__slow_poll_process_poll_req()`, on whichever thread is inside `uv_run()` — and, as above, nothing AOS registers takes that path anyway. Nothing this backend adds has a thread of its own either: a wait completion packet is queued to the port by the kernel and dequeued by the loop, and the `uv_check_t` is an ordinary loop callback. The only cross-thread entry point is `Quit()`, which is a `uv_async_send()` — the one libuv call documented as thread-safe — and whose work happens in the async's callback on the loop thread. `CallbacksRunOnTheLoopThread` pins it, on every backend: fifty rounds of a readiness callback, a raw completion in each direction, a timer and a wakeup, each checked against the thread that drove the loop.
...
- **A borrowed loop is still a kernel object.** Where libuv could not wait on what AOS needed, the port underneath it could, and using it the way the native backend uses its own turned a platform exclusion into a platform half. The same suite then found the two places libuv's own reports could not be taken at face value there, exactly as it had found the wording drifts on Linux.
...
- `aos/events/aio_uv_internal.h` — the seam; `aos/events/aio_uv_posix.cc` and `aos/events/aio_uv_windows.cc` — the platform halves
- `aos/events/aio_windows_internal.h` — the wait completion packet and kernel timer, shared with the IOCP backend
